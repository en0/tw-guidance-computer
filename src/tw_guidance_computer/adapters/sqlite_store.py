...
class SqliteGameStateStore(GameStateStore):
    ...
    def get_all_warps(self) -> list[WarpConnection]:
        rows = self._conn.execute("SELECT from_sector, to_sector, explored FROM warps").fetchall()
        return [WarpConnection(from_sector=r[0], to_sector=r[1], explored=bool(r[2])) for r in rows]
    ...
    def get_trade_pairs(self) -> list[TradePair]:
        # Delegate to the use case logic — this is a convenience method
        # that the CLI can call directly
